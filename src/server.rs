...
mod test {
    ...
    fn ntt_speed_test() {
        ...
        let trials = 1000;
        let max_val = 1 << 16;
        let mut gi_ct_ntts = vec![PolyMatrixNTT::zero(&params, 1, 1); trials];
        // let mut gi_cts = vec![PolyMatrixRaw::zero(&params, 1, 1); trials];

        for i in 0..trials {
            for z in 0..gi_ct_ntts[i].data.len() {
                gi_ct_ntts[i].data[z] = fastrand::u64(..max_val);
            }
        }
        ...
        println!(
            "to_ntt_no_reduce took {} us for {} trials",
