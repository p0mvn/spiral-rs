...
fn _interleave_rng_data(params: &Params, v_buf: &[u64], rng: &mut ChaCha20Rng) -> Vec<u64> {
    let mut out = Vec::new();
...
impl<'a> Query<'a> {
    ...
    pub fn deserialize(params: &'a Params, mut data: &[u8]) -> Self {
        ...
        if params.expand_queries {
            ...
            let mut v_ct = new_vec_raw(params, params.db_dim_1, 2, 2 * params.t_gsw);
            deserialize_vec_polymatrix_rng(&mut v_ct, &data, &mut rng);
            out.v_ct = Some(v_ct);
        }
        out
    }
...
impl<'a> Client<'a> {
    ...
    pub fn get_sk_gsw_full(&self) -> &PolyMatrixRaw<'a> {
        &self.sk_gsw_full
    }
    ...
    fn get_regev_sample(
        ...
        let e = PolyMatrixRaw::fast_noise(params, 1, 1, &self.dg, rng);
    ...
    fn get_scaled_regev_sample(
        ...
        let params = self.params;
        let a = PolyMatrixRaw::random_rng(params, 1, 1, rng_pub);
        let mut e = PolyMatrixRaw::fast_noise(params, 1, 1, &self.dg, rng);
        for i in 0..params.poly_len {
            e.data[i] = multiply_uint_mod(e.data[i], scale, params.modulus);
        }
        let b_p = &self.sk_reg.ntt() * &a.ntt();
        let b = &e.ntt() + &b_p;
        let mut p = PolyMatrixNTT::zero(params, 2, 1);
    ...
    fn get_fresh_scaled_reg_public_key(
        &self,
        m: usize,
        rng: &mut ChaCha20Rng,
        rng_pub: &mut ChaCha20Rng,
        ...
        for i in 0..m {
            p.copy_into(&self.get_scaled_regev_sample(rng, rng_pub, scale), 0, i);
        }
        p
    }

    pub fn encrypt_matrix_gsw(
        &self,
        ag: &PolyMatrixNTT<'a>,
        rng: &mut ChaCha20Rng,
        rng_pub: &mut ChaCha20Rng,
    ...
    pub fn encrypt_matrix_scaled_reg(
        &self,
        a: &PolyMatrixNTT<'a>,
        rng: &mut ChaCha20Rng,
        rng_pub: &mut ChaCha20Rng,
        ...
        let m = a.cols;
        let p = self.get_fresh_scaled_reg_public_key(m, rng, rng_pub, scale);
        &p + &a.pad_top(1)
    }
    ...
    pub fn generate_expansion_params(
        &self,
        num_exp: usize,
        m_exp: usize,
        rng: &mut ChaCha20Rng,
    ...
    pub fn negacyclic_sk_reg(&self) -> PolyMatrixRaw<'a> {
        let mut sk_reg = self.get_sk_reg().clone();

        (&mut sk_reg.as_mut_slice()[1..]).reverse();
        for z in 1..sk_reg.data.len() {
            sk_reg.data[z] = self.params.modulus - sk_reg.data[z];
        }
    ...
    pub fn generate_query(&self, idx_target: usize) -> Query<'a> {
        ...
        if params.expand_queries {
            // pack query into (possibly several) ciphertexts
            ...
            println!("num_query_cts: {}", num_query_cts);

            let mut cts = Vec::new();
            for idx_query in 0..num_query_cts {
                let num_exps = params.db_dim_1.min(MAX_EXP_DIM);

                let innner_idx_dim0 = if idx_query == (idx_dim0 / (1 << num_exps)) {
                    (idx_dim0 % (1 << num_exps)) as u64
                } else {
                    u64::MAX
                };

                let mut sigma = PolyMatrixRaw::zero(params, 1, 1);
                let inv_2_g_first = invert_uint_mod(1 << num_exps, params.modulus).unwrap();
                let inv_2_g_rest =
                    invert_uint_mod(1 << (params.stop_round() + 1), params.modulus).unwrap();

                if params.db_dim_2 == 0 || params.version == 3 {
                    for i in 0..(1 << num_exps) {
                        sigma.data[i].conditional_assign(
                            &scale_k,
                            (i as u64).ct_eq(&(innner_idx_dim0 as u64)),
                        )
                    }
                    ...
                    for i in 0..(1 << params.db_dim_1) {
                        sigma.data[2 * i].conditional_assign(
                            &scale_k,
                            (i as u64).ct_eq(&(innner_idx_dim0 as u64)),
                        )
                    }
                ...
                cts.push(
                    self.encrypt_matrix_reg(&sigma.ntt(), &mut rng, &mut rng_pub)
                        .raw(),
                );
            }
            query.v_ct = Some(cts);
        } else {
            // Upload only GSW ciphertexts
            assert_eq!(further_dims, 0);

            let mut sigma_v = Vec::<PolyMatrixNTT>::new();

            // generate GSW ciphertexts
            for i in 0..params.db_dim_1 {
                let rev_i = (params.db_dim_1 as u64) - 1 - (i as u64);
                let bit = ((idx_dim0 as u64) & (1 << rev_i)) >> rev_i;
                ...
                for j in 0..params.t_gsw {
                    ...
                    let ct = self.encrypt_matrix_reg(
                        &sigma_ntt,
                        &mut ChaCha20Rng::from_entropy(),
                        &mut ChaCha20Rng::from_entropy(),
                    );
                    ct_gsw.copy_into(&ct, 0, 2 * j + 1);
                    let prod = &self.get_sk_reg().ntt() * &sigma_ntt;
                    let ct = &self.encrypt_matrix_reg(
                        &prod,
                        &mut ChaCha20Rng::from_entropy(),
                        &mut ChaCha20Rng::from_entropy(),
            ...
            assert_eq!(query.v_ct.as_ref().unwrap().len(), params.db_dim_1);
        }
        query
    }
    ...
    pub fn decode_response(&self, data: &[u8], num_cts: usize) -> Vec<u8> {
        /*
        ...
        */
        ...
        result.to_vec(p_bits as usize, params.poly_len)
    }
}
